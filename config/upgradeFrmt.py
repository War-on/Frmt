...
''')
    ...
    while True:
        ...
        sleep(3)
        subprocess.run('cd ~/;git clone https://github.com/War-on/Frmt > /dev/null 2>&1', shell=True)
        if os.path.exists('/data/data/com.termux/files/home/Frmt/') == False:
            ...
            sleep(3)
            os.system('cd ~/Frmt;chmod 777 Frmt.py')
            print(f'\n>>{vd}[✔]{t}<< {vd}Atualização concluida.{t}')
            sleep(3)
            print(f'{a}\n[!] É preciso sair e entrar de novo para que as alterações sejam feitas.{t}')
            sleep(5)
            break
