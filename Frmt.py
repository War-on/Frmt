...
while True:
      if os.path.exists('/data/data/com.termux/files/home/Frmt/config/vcs_version.py') == True:
            ...
            if vcs_version.version == 'None':
                  ...
                  print(f'\n{a}[!] VERSÃO COMPATÍVEL:{t}')
                  print(f'{vd}\n[*] Termux versão 0.119.0{t} => ({az} https://encurtador.com.br/qBPQY {t})')
                  print(f'{vd}[*] Termux versção 0.118.0{t} => ({az} https://f-droid.org/repo/com.termux_118.apk {t})')
                  break

            elif vcs_version.version >= '0.118.0':
                  print(f'{vd}\n[*] ---[Versão ok-]---{t}')
                  sleep(5)
            elif vcs_version.result.returncode != 0:
                  print(f'{vm}\nError ao verificar a versão. Tente instalar uma versão mais recente do termux.{t}')
                  break
      ...
      if nm == '1' or nm == '01':
            ...
            if os.path.exists('/data/data/com.termux/files/home/sherlock') == False:
                  ...
                  if msg_error.returncode == 1:
                        msg = None
                        for line in msg_error.stdout.decode().split("\n"):
            ...
            animacao()
            print(f'{vd}[*] Instalando Kali Linux...{t}')
            sleep(3)
            subprocess.run('cd;wget -O install-nethunter-termux https://offs.ec/2MceZWr', shell=True)
            animacao()
            if os.path.exists('/data/data/com.termux/files/home/kali-arm64') == True:
                  print(f'{vd}[*] Kali ja instalado.')
                  break
            if os.path.exists('/data/data/com.termux/files/home/install-nethunter-termux') == False:
                  print(f'{vm}[x] Error ao instalar.{t}')
                  break
            print(f'{vd}Adicionado permissão...{t}')
            sleep(2)
            subprocess.run('cd;chmod +x install-nethunter-termux', shell=True)
            animacao()
            print(f'{vd}[*] Executando kali para instalação...{t}')
            sleep(3)
            animacao()
            subprocess.run('cd;./install-nethunter-termux', shell=True)
            subprocess.run('cd;rm -rf install-nethunter-termux', shell=True)
            print(f'\n{vd}Para usar o kali, basta utilizar o comando{t}({a}nh{t})')
            break
      elif nm == '98':
            print(f'{cn}#{t}'*27)
            ...
            sleep(3)
            subprocess.run('cd /sdcard;git clone https://github.com/NuevaGeneracionALB/aleatory-md.git', shell=True)
            input(f'\n{a}Eae, de boa? Vou tentar executar o bot pra voce agora, fica esperto porque se tudo der certo vai ser gerado um QR-code e voce vai precisar de um cell pra escanear, show?{t}\n\nA instalação do bot foi feita em: {az}/sdcard/aleatory-md \n\n{vd}Aperte enter para continuar...{t} ')
            subprocess.run('cd /sdcard/aleatory-md/;sh start.sh', shell=True)
            break
      elif nm == '5' or nm == '05':
            animacao()
            ...
            sleep(5)
            animacao()
            subprocess.run('cd $HOME/Frmt/config;bash T-style.sh', shell=True)
            break
      elif nm == '6' or nm == '06':
            animacao()
